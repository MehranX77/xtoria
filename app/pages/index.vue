        ...
        <UContainer dir="ltr" class="md:mt-44 mt-10">
            ...
            <div dir="rtl" class="grid lg:grid-cols-2 grid-cols-1 gap-x-4 lg:gap-y-0 gap-y-5 mt-5">
                <div class="bg-slate-100 dark:bg-slate-800 rounded-lg shadow p-4 h-fit">
                    ...
                    <UCarousel v-slot="{ item }" dir="ltr" class="mt-4" :items="homeData?.data?.newest" arrows loop
                        autoplay
                    ...
                    <UCarousel v-slot="{ item }" dir="ltr" class="mt-4" :items="homeData?.data?.special_discount" arrows
                        ...
                        <div
                            ...
                            <div dir="rtl" class="flex-col space-y-2 ">
                                <UBadge v-if="item?.discount > 0" size="md" color="error" variant="soft"
                                    class="text-sm ">50% OFF </UBadge>
                                <h2 class="font-bold md:text-start text-center md:mt-0 mt-3">{{ item?.product?.name }}
                                </h2>
                                <p class="font-bold line-clamp-2 md:text-start text-center"
                                    v-html="item?.product?.description" />
                                <span v-if="item?.discount"
                                    class="self-center text-xl line-through text-muted md:text-start text-center">{{
                                    item?.discount }} تومان</span>
                                <Countdown class="md:text-start text-center font-bold text-xl text-rose-500"
                                    :date="new Date('feb 14, 2026 16:50:30')" v-slot="{ hours, minutes, seconds }">{{
                                    hours }} : {{ minutes }} : {{ seconds }}</Countdown>
                                <UButton
                                    :to="{ name: 'products-slug', params: { slug: item?.slug }, query: { p_id: item?.id } }"
                                    dir="rtl" class="place-content-center lg:text-xl rounded-md" color="error"
                                    variant="subtle">{{ numberFormater(item?.price) }} تومان</UButton>
                            </div>
                        </div>
                    </UCarousel>
                </div>
            ...
            <div class="mt-5 bg-slate-50 dark:bg-slate-800 p-5 rounded-lg">
                <div class="flex justify-between">
                    <UButton to="/products" color="neutral" variant="ghost" size="xl"
                        icon="solar:arrow-left-bold-duotone" class="self-center text-md hover:cursor-pointer"><span
                            class="md:flex hidden">مشاهده همه</span></UButton>
            ...
            <div class="mt-5 bg-slate-50 dark:bg-slate-800 p-5 rounded-lg">
                <div class="flex justify-between">
                    <UButton disabled color="neutral" variant="ghost" size="xl" icon="solar:arrow-left-bold-duotone"
                        class="self-center text-md hover:cursor-pointer"><span class="md:flex hidden">مشاهده همه</span>
                    </UButton>
            ...
            <div class="my-8">
                ...
                <NuxtErrorBoundary>
                    <!-- defualt component -->
                    <LazyCategoryLists :category-list="homeData?.data?.categories" class="col-span-12"
                        hydrate-on-visible />
                    <!-- error handler -->
            ...
            <Transition mode="out-in" name="page">
                <Suspense>
                    <template #default>
                        <LazyBlogPost hydrate-on-visible />
                    </template>
                    <template #fallback>
                        <USkeleton class="w-65 h-70 rounded-lg "/>
                    </template>
                </Suspense>
            </Transition>
        </UContainer>
