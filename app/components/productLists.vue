<template>
    <div v-for="(item) in product?.data?.results" :key="item.id" class="md:col-span-2 col-span-1 flex flex-col gap-y-1">
        ...
        <UButton :to="{ name: 'products-slug', params: { slug: item?.slug } }" color="error" dir="rtl" size="lg" class="rounded-full place-content-center w-[86%] mx-auto mb-1">{{ numberFormater(item?.price) 
            }} تومن </UButton>
    </div>
</template>
